...
def analyze_numeric(baseline_raw, new_raw, thresholds=None):
    ...
    # Sample size warning
    ratio = max(len(baseline), len(new)) / max(min(len(baseline), len(new)), 1)
    if ratio > 10:
        warnings.append(
            f"Sample size mismatch: baseline={len(baseline)}, new={len(new)} "
    ...
    # Row counts
    metrics["baseline_count"] = len(baseline)
...
def analyze(baseline_cols, new_cols, columns=None, ignore_columns=None, thresholds=None):
    """
    Compare baseline and new column dicts.

    Args:
        baseline_cols: dict of column_name -> list of values
